...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        # Check if input is webcam (numeric string)
        if str(path).isdigit() or str(path).startswith('/dev/video'):
            self.webcam = True
            self.cap = cv2.VideoCapture(int(path) if str(path).isdigit() else path)
            if not self.cap.isOpened():
                raise Exception(f'ERROR: Cannot open webcam {path}. Check if camera is connected and permissions are correct.')
            self.nf = float('inf')  # Infinite frames for webcam
            self.files = [path]
    ...
    def __next__(self):
        ...
        if hasattr(self, 'webcam') and self.webcam:
            ...
            if not ret_val:
                ...
                raise StopIteration
            path = f'webcam_{self.count}'
            print(f'webcam {self.count}: ', end='')
            self.count += 1
            
            # For webcam, we need to handle the exit condition
